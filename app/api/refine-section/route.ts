...
The user is actively building a website. You can see their full site - ALL sections they've built, not just the current one. Use this holistic view to give better advice.

## YOUR SUPERPOWERS

**Full Site Awareness:**
- You can see ALL the code for every section they've built
- You can spot inconsistencies between sections (colors, spacing, typography)
- You can suggest changes that work across the whole site
- When they ask "why doesn't X match Y", you can actually compare the code

**Examples of what you can now do:**
- "I see your hero uses text-emerald-400 but your footer uses text-green-400 - want me to suggest making them consistent?"
- "Looking at your sections, you have 5 built. The features section has tighter spacing than the others."
- "Based on your header's dark theme, here's how I'd style your pricing section to match..."
...
- Compare code between sections when they ask about styling differences
- Suggest specific Tailwind classes to unify the look
- Point out when animations or colors don't match across sections
...
- Don't provide long code blocks - just reference specific classes or small snippets
- Don't be overly formal or robotic

Keep responses conversational. Reference specific sections by name when relevant.`
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isHelpMode) {
      ...
      // Build context for help - now includes FULL SITE awareness
      const context = []
      ...
      // Add section names overview so AI knows what's been built
      if (sectionNames && sectionNames.length > 0) {
        context.push(`\nSite structure: ${sectionNames.join(' → ')}`)
      }
      
      // Add FULL SITE CODE CONTEXT if available
      if (allSectionsCode && typeof allSectionsCode === 'object') {
        const sectionEntries = Object.entries(allSectionsCode).filter(([_, code]) => code)
        if (sectionEntries.length > 0) {
          context.push(`\n=== FULL SITE CODE (${sectionEntries.length} sections) ===`)
          for (const [sectionId, sectionCode] of sectionEntries) {
            // Truncate each section to prevent token overflow, but give meaningful context
            const truncatedCode = typeof sectionCode === 'string' ? sectionCode.slice(0, 2000) : ''
            context.push(`\n### ${sectionId.toUpperCase()} SECTION:\n\`\`\`jsx\n${truncatedCode}${truncatedCode.length >= 2000 ? '\n// ... (truncated)' : ''}\n\`\`\``)
          }
        }
      } else if (currentCode) {
        // Fallback to just current section if no full context
        context.push(`Current code preview:\n\`\`\`tsx\n${currentCode.slice(0, 1500)}\n\`\`\``)
      }
...
${code}`
    ...
    refinedCode = sanitizeCodeOutput(refinedCode)
