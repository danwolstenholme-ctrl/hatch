...
import { Plus, ArrowRight, Trash2, Search, Globe, LayoutGrid, List, ExternalLink, MoreHorizontal, Clock, ChevronRight, Zap, Crown, Check, RotateCcw } from 'lucide-react'
          ...
          <div className="flex items-center justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              {process.env.NODE_ENV === 'development' && (
                <button
                  onClick={() => {
                    if (confirm('Clear all hatch_* localStorage keys?')) {
                      Object.keys(localStorage)
                        .filter(k => k.startsWith('hatch_'))
                        .forEach(k => localStorage.removeItem(k))
                      window.location.reload()
                    }
                  }}
                  className="p-2 text-zinc-600 hover:text-orange-400 hover:bg-orange-500/10 rounded-lg transition-colors"
