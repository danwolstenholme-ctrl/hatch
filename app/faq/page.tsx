    ...
    questions: [
      {
        ...
        a: 'System access is tiered. Free Tier grants 5 generations/day. Starter Tier ($9/mo) unlocks unlimited generation for up to 3 sites. Pro Tier ($29/mo) grants unlimited generation, full deployment, and code export. Agency Tier ($99/mo) adds white-label capabilities and priority support.',
      },
      {
          ...
          <div className="flex flex-wrap gap-2 mb-16">
            {faqs.map((section) => (
              <button
                ...
                className="px-4 py-2 bg-zinc-900/50 hover:bg-zinc-900 border border-zinc-800 hover:border-emerald-500/30 rounded-sm text-xs font-mono text-zinc-400 hover:text-emerald-400 transition-all uppercase tracking-wider"
              >
                [{section.category}]
              </button>
            ))}
                  ...
                  <div className="space-y-4">
                    {section.questions.map((faq, idx) => {
                      ...
                      return (
                        <motion.div
                          key={idx}
                          className="bg-zinc-900/20 border border-zinc-800/50 rounded-sm overflow-hidden hover:border-emerald-500/30 transition-colors"
                          initial={{ opacity: 0, y: 20 }}
