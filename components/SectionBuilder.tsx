...
function AiUnderstoodSummary({ reasoning, isExpanded, onToggle }: {
  ...
  if (reasoning.startsWith('Generated by Claude') || reasoning === 'Raw output from Claude.') {
    return null
  }
  
  // Parse reasoning into bullet points
  // Handle various formats: newlines, bullets, numbered lists, or sentences
  const points = reasoning
    .split(/[\n•\d+\.\s]+(?=[A-Z\*])/)  // Split on newlines, bullets, numbers
    .flatMap(s => s.split(/\.\s+(?=[A-Z])/)) // Also split on sentence boundaries
    .map(s => s.replace(/^\*+\s*/, '').trim()) // Remove markdown bold markers
...
// =============================================================================
// COMMAND BAR - Unified builder interface for both demo and auth users
// =============================================================================
...
export default function SectionBuilder({
  ...
  // IMPORTANT: For auth users, wait for brandConfig to be loaded before auto-building
  // (Demo mode has brandConfig set immediately, but auth loads it from project)
  useEffect(() => {
    if (autoBuildRanRef.current) return
    // Don't auto-build for auth users until brandConfig is loaded
    // Demo mode (isDemo=true) has brandConfig set immediately in the default state
    if (!isDemo && !brandConfig) {
      console.log('[SectionBuilder] Waiting for brandConfig before auto-build...')
      return
    }
  ...
  }, [isDemo, brandConfig]) // Re-run when brandConfig becomes available
  ...
  // The Chronosphere: Log style evolution locally (client-side DNA)
  const evolveUserStyle = async (code: string) => {
    if (demoMode || !code) return
    try {
      // Use client-side chronosphere instead of non-existent API
      chronosphere.log('refinement', { codeLength: code.length }, section.id)
    } catch (e) {
...
${code.slice(0, 2000)}`,
  ...
  const handleBuildSection = async (options?: { skipGuestCredit?: boolean; overridePrompt?: string }) => {
    ...
    // Warn if brandConfig is missing (shouldn't happen normally)
    if (!isDemo && !brandConfig) {
      console.warn('[SectionBuilder] Building without brandConfig - colors may not be applied')
    }
    ...
    */
    ...
    try {
      ...
      // Debug: Log brandConfig being sent
      console.log('[SectionBuilder] brandConfig being sent:', JSON.stringify(brandConfig, null, 2))
  ...
  // Both auth and demo users see the same clean centered empty state
  // with CommandBar at the bottom
  
  return (
    <div className="relative w-full flex-1 min-h-0 bg-zinc-950 overflow-hidden flex flex-col">
      
      {/* GuestPromptModal removed - Demo mode now uses CommandBar in bottom panel */}
